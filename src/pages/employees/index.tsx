...
export function EmployeeListPage(props: EmployeeListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getEmployees({
        relations: ['user', 'organization', 'performance_evaluation.count', 'time_tracking.count'],
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('time_tracking', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          accessorKey: 'time_tracking',
          cell: ({ row: { original: record } }: any) => record?._count?.time_tracking,
        }
      : null,
